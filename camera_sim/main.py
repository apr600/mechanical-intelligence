...
dir_path = "/home/anon/ahalya/LearningSensoryStructure/camera-intensity/data/rgb/entropy/"
...
robot = Robot(x0=x0, wslim=lim, ts=dt, explr_idx=[0,1], buffer_capacity=10000,  target_dist=model)#, horizon=12)
...
for iter_step in range(num_steps):
    ...
    if iter_step > batch_size:
        ...
        for iter_opt in range(num_learning_opt):
            ...
            # print(y_pred.shape, y_pred.shape[1:], tuple(y_pred.shape[1:]), y_logvar.shape)
            # print(y_pred.shape, torch.add(y_pred).shape)
            p = Normal(z_mu, z_logvar.exp())
            q = Normal(y_pred, y_logvar.exp().unsqueeze(1).unsqueeze(2).unsqueeze(3))#img_logvar.exp())#.repeat(1,3,75,75))#tuple(y_pred.shape[1:])))

...
class Robot():
    ...
    def __init__(self, x0, wslim, explr_idx,  target_dist=None, ts=0.1, horizon=10, buffer_capacity=100, std=0.05):
        ...
        self.barrier = BarrierFunction(b_lim = [self.lim]*2, barr_weight=50.)
    ...
    def kldiv_planner(self, num_target_samples, num_traj_samples, R=1, alpha=1e-3):
        ...
        for i in range(self.horizon):
            ...
            A, B = self.planner.get_lin(self.planner.state.copy(), self.u[i]*0)
...
if __name__ == "__main__":
    ...
    for i in range(num_steps): 
        ...
        state, _ = robot.step(num_target_samples= 100, num_traj_samples=num_steps, R=0.01, alpha=1e-3) # Optimized for uniform
        path.append(state)
    path = np.array(path)

...
class VAE(nn.Module):
    ...
    def __init__(self, input_dim, z_dim, s_dim, img_dim, hidden_dim=128):
        ...
        self.img_decoder = nn.Sequential(
            UnFlatten(),
            nn.ConvTranspose2d(in_channels=img_dim,
                               out_channels=5,
                               kernel_size=5,
                               stride=1
                               ),
        ...
        self.imgvar_decoder = nn.Sequential(
            nn.Linear(z_dim+s_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, x, y):
        y_transform = self.encoder(y)
        z_out = self.encode(torch.cat([y_transform, x], dim=1))
        ...
        y_pred = self.img_decode(torch.cat([z_samples, x], dim=1))
        img_pred = self.img_decoder(y_pred)
        # y_out = self.img_decode(torch.cat([z_samples, x], dim=1))
        # y_pred = y_out[:,1:]
        # y_logvar = y_out[:,0].unsqueeze(axis=1)
        y_logvar = self.imgvar_decoder(torch.cat([z_samples, x], dim=1))
        img_logvar = torch.clamp(y_logvar, -5, 2)
        return  img_pred, img_logvar , z_mu, z_logvar, z_samples
    ...
    def update_dist(self, xr, y):
        with torch.no_grad():
           self.img_pred, self.img_logvar , self.z_mu, self.z_logvar, self.z_samples = self.forward(xr, y)
        self.init = True
    ...
    def pdf(self, samples=None):
        if not self.init:
            ...
            print("using entropy dist")
            var_data = [None]*len(samples)
            with torch.no_grad():
                xr = torch.FloatTensor(samples)#.to(device)
                z_samples = self.z_samples.repeat(xr.shape[0], 1)
                img_logvar = self.imgvar_decoder(torch.cat([z_samples, xr], dim=1))
                var_data = img_logvar.exp().detach().cpu().numpy().squeeze()
                var_data /= np.max(var_data)
            return var_data
 
if __name__ == "__main__":
    vae = VAE(input_dim=5120,  z_dim=6,s_dim=2,  img_dim=5, hidden_dim=16)
    ...
    z_out  = vae.encode(torch.cat([out, x], dim=1))
    print(z_out.shape)
    ...
    img_logvar = vae.imgvar_decoder(torch.cat([z_samples, x], dim=1))

...
class FrankaEnv(object):
    def __init__(self, render=False, ts=1./60., offset=[0.,0.,0.],
                 x0 = [0.,0.,0.]):
        ...
        flags = self.bullet_client.URDF_ENABLE_CACHED_GRAPHICS_SHAPES
        tableID = self.bullet_client.loadURDF(os.getcwd() + "/franka/urdf/table_plain.urdf", np.array( [0, -.75, -0.6])+self.offset, [-0.5, -0.5, -0.5, 0.5], flags=flags)

...
robot = Robot(x0=x0, wslim=lim, ts=dt, explr_idx=[0,1], buffer_capacity=10000,  target_dist=model, horizon=time_horizon)
...
for iter_step in range(num_steps):
    ### Plan in KL-Erg ###
    state, action = robot.step(num_target_samples= num_samples, num_traj_samples=num_steps, R=0.5, alpha=1e-3)
...
plt.plot(obj2_loc[0],obj2_loc[2], 'b*')
plt.axis("square")
